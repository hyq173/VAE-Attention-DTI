...
class TextEmbedding(nn.Module):
    def __init__(self, embedding, dropout=0.3, freeze=False, name='textEmbedding'):
        ...
        self.name = name
        self.embedding = nn.Embedding.from_pretrained(torch.tensor(embedding,dtype=torch.float32), freeze=freeze) # embedding:(64,128)[64:字典大小,128:嵌入向量大小], dropout=0.5 , freeze：若为True，表示训练过程不更新，默认为True
...
class TextCNN(nn.Module):
    def __init__(self, featureSize, filterSize, contextSizeList, reduction='pool', actFunc=nn.ReLU, bn=False, ln=False, name='textCNN'):
        ...
        for i in range(len(contextSizeList)):
            ...
            bns.append(nn.BatchNorm1d(filterSize))  # BatchNorm1d : 在深度神经网络训练过程中使得每一层神经网络的输入保持相同【分布】
...
class MLP(nn.Module):
    def __init__(self, inSize, outSize, hiddenList=[], dropout=0.0, bnEveryLayer=False, dpEveryLayer=False,
                 outBn=False, outAct=False, outDp=False, name='MLP', actFunc=nn.ReLU):
    ...
    def forward(self, x):
        for h,bn in zip(self.hiddens,self.bns):
            ...
            if self.dpEveryLayer:
                x = self.dropout(x)
        x = self.out(x)  # 【最终的处理 这里变成了1维(注意在网络的最后一部分)】
...
class GCN(nn.Module):
    def __init__(self, inSize, outSize, hiddenSizeList=[], dropout=0.0, bnEveryLayer=False, dpEveryLayer=False,
        ...
        self.outDp = outDp
        self.resnet = resnet
    def forward(self, x, L):  # x->node,L->PL
