...
class BaseClassifier:
    ...
    def train(self, dataClass, log, trainSize=256, batchSize=256, epoch=100, lr=0.001, stopRounds=10, earlyStop=10, saveRounds=1,
              optimType='Adam', preheat=5, lr1=0.001, lr2=0.00003, momentum=0.9, weightDecay=0, isHigherBetter=True, 
              metrics="AUC", report=["ACC", "AUC", "Precision", "Recall", "F1"], 
              savePath='model'):
        dataClass.describe()
        ...
        schedulerRLR = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max' if isHigherBetter else 'min', factor=0.5, patience=4, verbose=True)  # 根据测试指标对学习率的调整
        ...
        for e in range(epoch):
            if e==preheat:
                ...
                schedulerRLR = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max' if isHigherBetter else 'min', factor=0.5, patience=30, verbose=True) # 学习率的调整
            ...
            if dataClass.validSampleNum>0 and (e+1)%saveRounds==0:
                ...
                metrictor(report)
                save_log(log, f'[Total Train]'+str(metrictor(report)))
                ...
                save_log(log, f'[Total Valid]'+str(metrictor(report)))
                mtc = res[metrics]
        ...
        metrictor.set_data(Y_pre, Y)
        metrictor(report)
        save_log(log, f'[Total test]'+str(metrictor(report)))
    ...
    def save(self, path, epochs, bestMtc=None, dataClass=None):
        ...
        for module in self.moduleList:
            stateDict[module.name] = module.state_dict() # 模型保存时使用
    ...
    def _train_step(self, X, Y, optimizer):
        ...
        if p:
            nn.utils.clip_grad_norm_(self.moduleList.parameters(), max_norm=20, norm_type=2)  # 对一组可迭代(网络)参数的梯度范数进行裁剪。效果如同将所有参数连接成单个向量来计算范数。
            optimizer.step()
            optimizer.zero_grad()  # 新的batch中梯度置为0
            #self.schedulerWU.step_and_update_lr()
            #self.schedulerWU.zero_grad()
        return loss*self.stepUpdate

class DTI_E2E(BaseClassifier):
    def __init__(self, amEmbedding, goEmbedding, atEmbedding,
    # def __init__(self, amEmbedding, goEmbedding, atEmbedding, seqMaxLen,
                 ...
                 embFreeze=False, sampleType='PWRL', device=device):  # 把cuda换成CPU
        ...
        # self.dGCN = GCN(atEmbedding.shape[0], atEmbedding.shape[1], gcnHiddenSize, [gcnHiddenSize]*(gcnSkip-1), name='dGCN').to(device) # 原来的结果
        self.dGCN = GCN(atEmbedding.shape[0], atEmbedding.shape[1], [gcnHiddenSize]*(gcnSkip-1), name='dGCN').to(device)
...
class DTI_E2E_nogo(BaseClassifier):
    def __init__(self, amEmbedding, atEmbedding,
...
class DTI_Bridge(BaseClassifier):
    def __init__(self, outSize, 
        ...
        self.criterion = nn.BCEWithLogitsLoss()  # BinaryCrossEntropyLoss
    ...
    def calculate_y_logit(self, X, mode='train'):
        ...
        if self.nodeNum>0:
            node = self.nodeEmbedding.dropout2(self.nodeEmbedding.dropout1(self.nodeEmbedding.embedding.weight)).repeat(len(Xat), 1, 1) # node.xlsx(128,64,128)
            node = torch.cat([Xam, Xat, node], dim=1) # => batchSize × nodeNum × outSize ,node.xlsx(128,66,128)
            nodeDist = torch.sqrt(torch.sum(node**2,dim=2,keepdim=True)+1e-8)# => batchSize × nodeNum × 1 ,nd(128,66,1),keepdim=T:保持维度一致

            cosNode = torch.matmul(node,node.transpose(1,2)) / (nodeDist*nodeDist.transpose(1,2)+1e-8) # => batchSize × nodeNum × nodeNum (128,66,66)
            #cosNode = cosNode*0.5 + 0.5
            cosNode = F.relu(cosNode) # => batchSize × nodeNum × nodeNum  (128,66,66) cosNode作为邻接矩阵
            cosNode[:,range(node.shape[1]),range(node.shape[1])] = 1 # => batchSize × nodeNum × nodeNum (66,66)矩阵的对角线置为1 同一个向量之间的夹角是0
            if self.maskDTI: cosNode[:,0,1] = cosNode[:,1,0] = 0
            D = torch.eye(node.shape[1], dtype=torch.float32, device=self.device).repeat(len(Xam),1,1) # => batchSize × nodeNum × nodeNum (128,66,66)
            D[:,range(node.shape[1]),range(node.shape[1])] = 1/(torch.sum(cosNode,dim=2)**0.5) # 对角线元素 这里的D作为度矩阵
            pL = torch.matmul(torch.matmul(D,cosNode),D) # => batchSize × batchnodeNum × nodeNumSize (128,66,66) PL=D^(-1/2) * cosNode * D^(-1/2)
            node_gcned = self.nodeGCN(node, pL) # => batchSize × nodeNum × outSize (128,66,128)
            # node_embed的第一行数据是Protein,第二行是drug
            node_embed = node_gcned[:,0,:]*node_gcned[:,1,:] # => batchSize × outSize (128,128)
        else:
